*Este projeto foi criado como parte do currículo da 42 por [gabde-so](https://profile-v3.intra.42.fr/users/gabde-so)*
...
### 2. BFS: valida se existe caminho
...
### 3. Dijkstra: caminho mais barato em turnos
`BFS` só conta saltos; `Dijkstra` (com `heapq`) encontra o caminho de menor custo em turnos, já que `restricted` custa 2 turnos e `normal`/`priority` custam 1. Zonas `blocked` nunca são expandidas.
